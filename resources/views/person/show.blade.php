...
@section('title')
    {!! implode(', ', array_filter([$person->last, $person->first])) !!}
@endsection
    ...
    <div class="container-fluid">
        <div class="d-flex">
            <div class="flex-grow-1">
                <h1>{!! implode(', ', array_filter([$person->last, $person->first])) !!}</h1>
            </div>
            @auth
            <div>
                <a class="btn btn-text text-info" href="{{ route('person.edit', $person->id) }}" title="Edit this person"><i class="far fa-edit"></i></a>
            </div>
            <div>
                <form action="{{ route('person.destroy', $person->id) }}" method="POST">
                    @method('DELETE')
                    @csrf
                    <button class="btn btn-text text-danger" title="Delete this person"><i class="far fa-trash-alt"></i></button>
                </form>
            </div>
            @endauth
        </div>
        ...
        <div class="my-4">
            <div class="row">
                <div class="col-3 col-md-2">Person ID:</div>
                <div class="col">{{ $person->id }}</div>
            </div>
            <div class="row">
                <div class="col-3 col-md-2">Roles:</div>
                <div class="col">
                    @if ($person->roles->isEmpty())
                        None
                    @else
                        {!! $person->roles->pluck('name')->implode('<br/>') !!}
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-auto">
                ...
                <a href="{{ route('person.export', ['person' => $person->id, 'format' => 'csv', 'csv.format' => 'graph']) }}" type="button" class="btn btn-lg btn-link" title="Export CSV data">
                    <i class="fas fa-file-csv"></i>
                </a>
                <a href="{{ route('person.export', ['person' => $person->id, 'format' => 'xml']) }}" target="export" type="button" class="btn btn-lg btn-link" title="Export XML data">
                    <i class="fas fa-file-code"></i>
                </a>
                <a href="{{ route('person.export', ['person' => $person->id, 'format' => 'graphviz']) }}" target="export" type="button" class="btn btn-lg btn-link" title="Export Graphviz data">
