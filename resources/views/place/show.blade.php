...
@section('content')
    <div class="container-fluid">
        <div class="d-flex">
            ...
            @auth
                <div>
                    <a class="btn btn-text text-info" href="{{ route('place.edit', $place->id) }}" title="Edit this place"><i class="far fa-edit"></i></a>
                </div>
                <div>
                    <form action="{{ route('place.destroy', $place->id) }}" method="POST">
                        @method('DELETE')
                        @csrf
                        <button class="btn btn-text text-danger" title="Delete this place"><i class="far fa-trash-alt"></i></button>
                    </form>
                </div>
            @endauth
        </div>
        ...
        <div class="my-4">
            <div class="row">
                <div class="col-3 col-md-2">Place ID:</div>
                <div class="col">{{ $place->id }}</div>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-auto">
                ...
                <a href="{{ route('place.export', ['place' => $place->id, 'format' => 'csv', 'csv.format' => 'graph']) }}" type="button" class="btn btn-lg btn-link" title="Export CSV Graph data">
                    <i class="fas fa-file-csv"></i>
                </a>
                <a href="{{ route('place.export', ['place' => $place->id, 'format' => 'xml']) }}" target="export" type="button" class="btn btn-lg btn-link" title="Export XML data">
                    <i class="fas fa-file-code"></i>
                </a>
                <a href="{{ route('place.export', ['place' => $place->id, 'format' => 'graphviz']) }}" target="export" type="button" class="btn btn-lg btn-link" title="Export Graphviz data">
